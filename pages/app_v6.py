...
_user_name     = _current_user.get("name","") or _current_user.get("email","User")
_user_avatar   = _current_user.get("avatar","")
...
with st.sidebar:
    ...
    st.divider()
    if _user_avatar:
        st.markdown(f"<img src='{_user_avatar}' style='width:32px;height:32px;border-radius:50%;margin-bottom:6px'>", unsafe_allow_html=True)
    st.markdown(f"<p style='font-size:0.8rem;color:#9ca3af;margin:0'>{_user_name}</p>", unsafe_allow_html=True)
    if st.button("Sign out", use_container_width=True):
        _auth.logout()
    st.caption("Nexus · v0.3")
...
if _food_available:
    try:
        food_events = _fl.food_log_to_context_events(days=30, user_id=_user_id)
    except Exception:
        food_events = {}
...
if _food_available:
    try:
        ...
        _fdf = _fl2.load_food_log(days=30, user_id=_user_id)
    ...
    with tab:
        ...
        fig.update_layout(
            ...
            yaxis=dict(gridcolor="#1a1d26",
                       title=dict(text=f"{cfg['label']} ({cfg['unit']})",
