...
_current_user = _auth.require_login()
...
with cap_col:
    ...
    # Auto-extract timestamp from EXIF
    if img_bytes:
        photo_timestamp = fl.extract_photo_timestamp(img_bytes)
        if photo_timestamp:
            st.markdown(
                f"<p style='font-size:0.78rem;color:#34d399;margin:4px 0 8px'>"
                f"📍 Photo taken at {photo_timestamp.strftime('%-I:%M %p')}"
                f" · {photo_timestamp.strftime('%b %-d')}</p>",
                unsafe_allow_html=True
            )
            meal_time_override = photo_timestamp.time()
        else:
            st.markdown(
                "<p style='font-size:0.78rem;color:#6b7280;margin:4px 0 8px'>"
                "No EXIF timestamp found — set time manually below</p>",
                unsafe_allow_html=True
            )
...
with result_col:
    if img_bytes and analyze_btn:
        ...
        saved = fl.save_food_log(analysis, logged_at=logged_at, user_id=_user_id)
        if saved:
...
def get_food_data(days, uid):
    return fl.load_food_log(days=days, user_id=uid)

food_df = get_food_data(days_back, _user_id)
